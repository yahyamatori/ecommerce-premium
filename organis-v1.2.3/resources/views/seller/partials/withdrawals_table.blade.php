	...
	<table class="table table-borderless table-theme" style="width:100%;">
		...
		<tbody>
			...
			<tr>
				...
				<td class="text-left">{{ $gtext['currency_icon'] }}{{ NumberFormat($row->amount) }}</td>
				@else
				<td class="text-left">{{ NumberFormat($row->amount) }}{{ $gtext['currency_icon'] }}</td>
				@endif
				
				@if($gtext['currency_position'] == 'left')
				<td class="text-left">{{ $gtext['currency_icon'] }}{{ NumberFormat($row->fee_amount) }}</td>
				@else
				<td class="text-left">{{ NumberFormat($row->fee_amount) }}{{ $gtext['currency_icon'] }}</td>
				@endif
